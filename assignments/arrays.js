...
const found = []; 
inventory.forEach((model) => {if (model.id === 33) {found.push(model.car_year, model.car_make, model.car_model)}});
console.log('Car 33 is a ' + found[0] + " " + found[1] + " " + found[2]);
...
let carModels = [];
inventory.forEach(car => carModels.push(car.car_model));  
...
let carYears = [];
inventory.forEach(car => carYears.push(car.car_year));  
...
let oldCars = [];
carYears.forEach((thisCar) => {if (thisCar < 2000) {oldCars.push(thisCar)}});
...
let BMWAndAudi = [];
inventory.forEach((model) => {if (model.car_make === 'BMW' || model.car_make === 'Audi') {BMWAndAudi.push(model)}});
